...
class Screen:
    ...
    def display_object(self, item, elem):
        elem.fill([20, 220, 20])
        myfont = Font(None, int(self.sprite_size * 1.2))
        txt_surface = myfont.render(item, 1, (0, 0, 0))
        txt_size = myfont.size(item)
        elem.blit(
            txt_surface,
            (
                (self.sprite_size - txt_size[0]) // 2,
                (self.sprite_size - txt_size[1]) // 2 + 1,
            ),
        )
    ...
    def display_game(self):
        ...
        for i in self.labyrinthe.player.items:
            ...
            footer.blit(
                txt_surface,
                (
                    int(i) * sprite_size - (txt_size[0] // 2),
                    (sprite_size - txt_size[1]) // 2 + 1,
                ),
            )
    ...
    def display_init(self):
        ...
        self.window.blit(
            txt_surface,
            (
                (self.window.get_width() - txt_size[0]) // 2,
                (self.window.get_height() - txt_size[1]) // 2,
            ),
        )
